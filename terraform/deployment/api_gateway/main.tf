...
resource "aws_ecs_task_definition" "api_gateway_task_definition" {
  ...
  container_definitions = jsonencode([
    {
      name         = var.api_gateway_name
      image        = var.api_gateway_ecr_image
      essential    = true
      portMappings = [{ containerPort = var.api_gateway_container_port }]
      logConfiguration = {
        logDriver = "awslogs"
        options = {
...
resource "aws_ecs_service" "api_gateway_ecs_service" {
  ...
  network_configuration {
    ...
    security_groups  = [data.terraform_remote_state.shared_modules.outputs.api_gateway_sg_id]
